        ...
        >
            <GlassCard 
                ...
                className={`
                    ...
                    border-zinc-200 dark:border-white/10 transition-all duration-300 
                    ${cert.border}
                        ...
                        {cert.items.map((item, i) => (
                            <li 
                                ...
                                className={`flex justify-between items-start gap-4 ${
                                    isGrid 
                                    ? 'bg-transparent p-3 rounded-lg transition-colors' 
                                    : 'pb-2 border-b border-zinc-100 dark:border-white/5 last:border-0 last:pb-0'
                                }`}
                            >
                                ...
                                <span className={`shrink-0 text-[10px] font-mono font-bold text-zinc-500 bg-zinc-100 dark:bg-white/5 px-1.5 py-0.5 rounded ${isGrid ? '' : 'mt-0.5'}`}>
                                    {item.year}
                                </span>
                            </li>
                        ))}
...
function EventCard({ event, isBackCard = false, isGlass = false }: { event: SciEvent, isBackCard?: boolean, isGlass?: boolean }) {
    ...
    const bgStyle = isGlass
        ? "bg-white/90 dark:bg-[#0c0c0e]/90 backdrop-blur-xl border border-white/20 dark:border-white/10"
        : "bg-white dark:bg-[#0c0c0e] border border-zinc-200 dark:border-white/10";

    const featuredBorder = isFeatured
        ? "ring-1 ring-amber-500/50 shadow-[0_20px_50px_-12px_rgba(245,158,11,0.2)]" 
        : "shadow-[0_20px_50px_-12px_rgba(0,0,0,0.15)] dark:shadow-[0_20px_50px_-12px_rgba(0,0,0,0.5)]";

    return (
        <div className={`
            w-full h-full rounded-2xl select-none flex flex-col overflow-hidden transition-all duration-500
            ${bgStyle}
            ${isBackCard ? 'grayscale opacity-70 pointer-events-none' : ''}
            ${featuredBorder}
        `}>
            ...
            <div className="p-5 md:p-6 pb-20 md:pb-6 flex flex-col h-full relative">
                ...
                <div className="flex flex-col gap-2 mb-4 md:mb-6 text-xs text-zinc-500 dark:text-zinc-400">
                    <div className="flex items-center gap-2">
                        <Mic className={`h-4 w-4 ${isFeatured ? 'text-amber-500' : 'text-purple-500'}`} />
                        <span className="font-medium text-zinc-700 dark:text-zinc-300">{event.role}</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <MapPin className="h-4 w-4 text-zinc-400" /> <span>{event.location}</span>
                    </div>
                </div>

                <div className="relative mb-4 flex-1">
                    <Quote className={`absolute -top-2 -left-1 h-6 w-6 opacity-20 ${isFeatured ? 'text-amber-500' : 'text-purple-500'}`} />
                    <p className={`text-xs md:text-sm text-zinc-600 dark:text-zinc-300 leading-relaxed pl-6 border-l-2 line-clamp-4 md:line-clamp-5 ${isFeatured ? 'border-amber-500/30' : 'border-purple-500/20'}`}>
                        {event.description}
                    </p>
                </div>
                ...
                {event.links && event.links.length > 0 && (
                     <div className="mt-auto pt-3 md:pt-4 border-t border-zinc-100 dark:border-white/5 flex flex-wrap gap-3">
                         {event.links.map((link, i) => (
                             <a key={i} href={link.href} target="_blank" className={`flex items-center gap-1 text-xs font-bold uppercase tracking-wide ${isFeatured ? 'text-amber-600' : 'text-emerald-500'} hover:underline`}>

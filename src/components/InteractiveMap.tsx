...
function MapController({ externalSelectedId, markers, onMarkerSelected }: { externalSelectedId: string | null, markers: MapMarker[], onMarkerSelected: (marker: MapMarker | null) => void }) {
  ...
  useEffect(() => {
    ...
    if (externalSelectedId === "reset") {
      ...
      map.flyTo([25, -25], 2, { duration: 2, easeLinearity: 0.25 });
      return;
    }
  ...
  const html = `
    <div style="position: relative; width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; opacity: ${opacity};">
      ...
      <div style="
        ...
        border-radius: 50%; box-shadow: 0 2px 6px rgba(0,0,0,0.5); transition: all 0.3s ease; z-index: 10;
      "></div>
    </div>
  `;
  return L.divIcon({ className: "custom-pulse-marker", html: html, iconSize: [36, 36], iconAnchor: [18, 18], popupAnchor: [0, -18] });
};
  ...
  return (
    <div className="flex flex-col items-center w-full h-full">
      <div className="relative flex-1 w-full rounded-2xl overflow-hidden border border-zinc-200 dark:border-white/10 shadow-xl z-0 bg-zinc-900">
        <MapContainer
          center={[25, -25]} zoom={2} scrollWheelZoom={false}
          style={{ height: "100%", width: "100%", zIndex: 0 }}
          minZoom={2} maxZoom={17} maxBounds={[[-85, -180], [85, 180]]}
        ...
        >
          ...
          {customMarkers.map((marker) => {
            ...
            return (
              <Marker
                key={marker.id} position={[marker.lat, marker.lon]}
        ...
        <AnimatePresence mode="wait">
          {activePopup && (
            <motion.div
              initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} transition={{ duration: 0.2 }}
            ...
            >
              <div className="pointer-events-auto relative p-5 rounded-xl bg-white/90 dark:bg-zinc-900/90 backdrop-blur-md border border-zinc-200 dark:border-white/10 shadow-2xl shadow-black/40">
                {selectedMarker && (
                  <button onClick={(e) => { e.stopPropagation(); setSelectedMarker(null); setHoveredMarker(null); }} className="absolute top-3 right-3 p-1 rounded-full bg-zinc-100 dark:bg-white/5 hover:bg-zinc-200 dark:hover:bg-white/10 text-zinc-500 transition-colors" >
                    <X className="h-4 w-4" />
                  </button>
                )}

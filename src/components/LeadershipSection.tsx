...
import { Handshake, GraduationCap, Calendar, MapPin, Briefcase, ChartGantt, Rocket, BrainCircuit, Gamepad2, BookOpen, MessageCircleHeart, Globe, HeartHandshake, ChevronDown, X, Mic, Medal, Plus, Building2, FileText, Compass, Users, ShieldCheck, Flag } from "lucide-react";
...
const CompanyLogo = ({ job, isModal = false }: { job: any, isModal?: boolean }) => {
  ...
  return (
    <div className={`flex items-center justify-center shrink-0 rounded-xl shadow-sm bg-white dark:bg-white/5 border border-zinc-200/50 dark:border-white/10 ${heightClass} ${isMulti ? 'w-auto px-2.5 gap-1.5' : `${singleWidthClass} p-1.5`}`}>
      {isMulti ? (
        <div className="flex items-center -space-x-2">
          {job.logos.map((logo: string, idx: number) => (
            <div key={idx} className="relative z-10 h-7 w-7 md:h-8 md:w-8 rounded-full ring-2 ring-white dark:ring-[#1a1a1c] bg-white p-0.5 shadow-sm flex items-center justify-center overflow-hidden" style={{ zIndex: 10 - idx }}>
              <img src={logo} alt="Publisher Logo" className="h-full w-full object-contain rounded-full" />
            </div>
          ))}
        </div>
      ) : (
        <img src={job.logo || (job.logos && job.logos[0])} alt={job.company} className="w-full h-full object-contain rounded-lg" />
      )}
    </div>
  );
};
          ...
          <div className="mb-8 md:mb-12 flex items-center justify-between gap-2 min-w-0">
            <div className="flex flex-col gap-0.5 min-w-0">
              ...
              <h2 className="text-xl md:text-4xl font-extrabold flex items-center tracking-tight text-zinc-900 dark:text-white gap-2 md:gap-3 min-w-0">
                <motion.div animate={{ scale: [1, 1.1, 1] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }} className="relative p-2 md:p-3 rounded-2xl bg-white/80 dark:bg-white/5 border border-zinc-200 dark:border-white/10 shadow-sm backdrop-blur-md group shrink-0" >
            ...
            <div className="flex items-center gap-1.5 md:gap-3 shrink-0">
              ...
              <NavButtonWithTooltip href="#timeline" icon={ChartGantt} text="Timeline" tooltip="Visual roadmap of my academic and leadership career since 2008" colorClass="border-blue-500/5 bg-blue-500/5 hover:bg-blue-500/10 text-blue-700 dark:text-blue-400" />
            </div>
          </div>
        </MotionWrapper>
          ...
          {LeadershipExperience.map((job: any, idx: number) => {
            ...
            return (
              <motion.div key={idx} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: idx * 0.1 }} viewport={{ once: true }} >
                <div onClick={() => setSelectedJob(job)} className="relative rounded-xl border bg-zinc-50 dark:bg-white/5 border-zinc-200 dark:border-white/10 hover:border-emerald-500/30 transition-all duration-300 overflow-hidden cursor-pointer p-4 active:scale-[0.98]" >
                  <div className="absolute top-3 right-3"><ChevronDown className="w-3.5 h-3.5 text-zinc-400" /></div>
                  <div className="flex items-start gap-3 pr-6">
                    <CompanyLogo job={job} />
                    <div className="flex-1 min-w-0">
                      <h3 className="text-[15px] font-bold text-zinc-900 dark:text-white leading-tight">{job.position}</h3>
                      <p className="text-[10px] text-emerald-600 dark:text-emerald-400 font-medium mt-0.5 mb-0 flex items-center gap-1"><Building2 className="w-3 h-3 shrink-0" /> {job.company}</p>
        ...
        <div className="hidden lg:block overflow-x-auto overflow-y-visible py-12 pl-4 -ml-4 pr-12 pb-24">
          <div className="flex items-start min-w-max gap-6">
            {LeadershipExperience.map((job: any, idx: number) => {
              ...
              const locationDisplay = typeof job.location === 'string' ? job.location : job.location.city;
              return (
                <motion.div key={idx} initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: idx * 0.1 }} viewport={{ once: true }} style={{ zIndex }} onClick={() => setSelectedJob(job)} className={`relative flex-shrink-0 cursor-pointer w-[330px] ${idx > 0 ? "-ml-24" : ""} transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] group hover:z-50 hover:translate-x-23 hover:-translate-y-5 hover:scale-[1.02]`}>
                  ...
                  <div className="p-5 relative overflow-hidden rounded-2xl h-[210px] flex flex-col justify-between bg-zinc-200/10 dark:bg-black/1 backdrop-blur-md border border-zinc-200/40 dark:border-emerald-500/30 transition-all duration-500 group-hover:bg-zinc-100/60 group-hover:dark:bg-black/40 group-hover:backdrop-blur-xl group-hover:border-emerald-500/70 group-hover:shadow-[0_20px_40px_rgba(16,185,129,0.15)]">
                    <div className="mb-4 mt-2">
                      <div className="flex items-start gap-4 mb-4">
                        ...
                        <div className="min-w-0 flex-1">
                          <h3 className="text-[17px] font-bold leading-tight text-zinc-900 dark:text-white mb-1 line-clamp-2">{job.position}</h3>
                          <p className="text-xs text-emerald-600 dark:text-emerald-400 font-medium truncate flex items-center gap-1"><Building2 className="w-3.5 h-3.5 shrink-0" /> {job.company}</p>
                        </div>
                      </div>
                    ...
                    <div className="pt-3 border-t border-zinc-200/40 dark:border-white/10 flex items-center justify-between text-zinc-400 dark:text-zinc-500 group-hover:text-emerald-400 transition-colors">
                      <span className="text-[9px] font-mono font-bold uppercase tracking-widest flex items-center gap-1.5"><Briefcase className="w-3 h-3" /> Explore Record</span>
                      <motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }} className="w-5 h-5 rounded-full bg-white/50 dark:bg-white/5 flex items-center justify-center text-blue-400 border border-blue-500/30 group-hover:bg-blue-500/30 transition-colors"><Plus className="w-3 h-3" /></motion.div>
                    </div>
                  </div>
                </motion.div>
              );
        ...
        <AnimatePresence>
          {selectedJob && (
            <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSelectedJob(null)} className="absolute inset-0 bg-black/80 backdrop-blur-sm cursor-pointer" />
              <motion.div initial={{ scale: 0.95, y: 20, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} exit={{ scale: 0.95, y: 20, opacity: 0 }} className="relative w-full max-w-sm md:max-w-3xl z-10 max-h-[95vh] flex flex-col" >
                ...
                <GlassCard className="flex flex-col w-full rounded-2xl overflow-hidden border border-zinc-200 dark:border-emerald-500/40 bg-zinc-50 dark:bg-[#0c0c0e] relative p-5 md:p-6 shadow-2xl text-left">
                  <button onClick={() => setSelectedJob(null)} className="absolute top-3 right-3 md:top-4 md:right-4 z-30 p-2 rounded-full bg-zinc-200 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-300 dark:hover:bg-zinc-700 transition-colors shadow-sm" aria-label="Close modal">
                  ...
                  <p className="text-xs md:text-sm text-emerald-600 dark:text-emerald-400 font-medium mb-3 pb-2 border-b border-zinc-200 dark:border-white/10 flex items-center gap-1.5">
                    <Building2 className="w-3.5 h-3.5 shrink-0" /> {selectedJob.company} • {typeof selectedJob.location === 'string' ? selectedJob.location : selectedJob.location.city}
                  </p>

                  <div className="space-y-3 overflow-y-auto max-h-[75vh] no-scrollbar pr-2 pb-4">
                    
                    {/* THESIS CARD PREMIUM */}
                    {selectedJob.thesisImage && (
                      <div className="relative rounded-2xl overflow-hidden border border-zinc-200/80 dark:border-emerald-500/30 bg-white/80 dark:bg-[#121215] shadow-lg mb-4 flex flex-col md:flex-row items-stretch">
                        <div className="relative w-full md:w-48 h-40 md:h-auto shrink-0 overflow-hidden bg-zinc-100 dark:bg-zinc-900 border-b md:border-b-0 md:border-r border-zinc-200 dark:border-white/15">
                          <img 
                            src={selectedJob.thesisImage} 
                            alt="Thesis Preview" 
                            className="w-full h-full object-cover object-top scale-105 filter blur-[1px]" 
                          />
                        </div>
                        <div className="p-4 md:p-5 flex flex-col justify-between flex-1">
                          <div>
                            <div className="flex items-center justify-between gap-2 mb-2">
                              <span className="text-[9px] font-mono uppercase tracking-widest px-2.5 py-1 rounded-md bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 font-bold">
                                Master&apos;s Thesis Supervision
                              </span>
                            </div>
                            <h4 className="text-sm md:text-base font-bold text-zinc-900 dark:text-white leading-snug">
                              {selectedJob.thesisTitle}
                            </h4>
                          </div>
                          <div className="mt-4 pt-3 border-t border-zinc-200 dark:border-white/10 flex items-center justify-between text-[11px] text-zinc-500 dark:text-zinc-400">
                            <span className="flex items-center gap-1.5 font-medium text-emerald-600 dark:text-emerald-400">
                              <GraduationCap className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
                              <span>Ricardo Sampaio</span>
                            </span>
                          </div>
                        </div>
                      </div>
                    ...
                    {selectedJob.achievements.map((ach: any, i: number) => {
                      ...
                      const isScout = selectedJob.company === "Corpo Nacional de Escutas";
                      const ScoutIcons = [Users, Compass, ShieldCheck];
                      const ScoutIcon = ScoutIcons[i % ScoutIcons.length];
                      ...
                      return (
                        <div key={i} className="flex items-start gap-2.5 text-xs md:text-sm text-zinc-600 dark:text-zinc-300 font-light leading-tight">
                          {isScout ? (
                            <ScoutIcon className="w-4 h-4 text-emerald-500 mt-0.5 shrink-0" />
                          ) : (
                    ...
                    {/* BARRA DE PROGRESSÃO UNIFICADA COM ALINHAMENTO EXATO E NÉON SUBUIL */}
                    {selectedJob.company === "Corpo Nacional de Escutas" && (
                      <div className="mt-4 pt-4 border-t border-zinc-200 dark:border-white/15">
                        <div className="flex items-center justify-between mb-3">
                          <h4 className="text-[10px] font-bold uppercase tracking-[0.15em] text-zinc-800 dark:text-zinc-300 flex items-center gap-2">
                            <Flag className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> 13-Year Track Record
                          </h4>
                        </div>

                        {/* Barra Contínua com Efeito Néon na respetiva cor ao passar o rato */}
                        <div className="relative w-full h-3 bg-zinc-200 dark:bg-zinc-800 rounded-full overflow-hidden flex shadow-inner mb-3">
                          <div className="h-full bg-emerald-500 w-[30.7%] transition-all duration-300 hover:shadow-[0_0_15px_#34d399] hover:brightness-125 cursor-pointer relative z-10" title="Exploradores (2008-2012)" />
                          <div className="h-full bg-blue-500 w-[30.7%] transition-all duration-300 hover:shadow-[0_0_15px_#60a5fa] hover:brightness-125 cursor-pointer relative z-10 border-x border-black/20" title="Pioneiros (2012-2016)" />
                          <div className="h-full bg-rose-600 w-[38.6%] transition-all duration-300 hover:shadow-[0_0_15px_#fb7185] hover:brightness-125 cursor-pointer relative z-10" title="Caminheiros (2016-2021)" />
                        </div>

                        {/* Rótulos com larguras exatas em percentagem alinhados com o início de cada secção da barra */}
                        <div className="flex text-[10px]">
                          <div className="w-[30.7%] text-left pl-0.5">
                            <span className="block text-emerald-500 font-bold">Exploradores</span>
                            <span className="text-[9px] font-mono text-zinc-500 dark:text-zinc-400">Oct 2008 - Oct 2012</span>
                          </div>
                          <div className="w-[30.7%] text-left pl-2">
                            <span className="block text-blue-500 font-bold">Pioneiros</span>
                            <span className="text-[9px] font-mono text-zinc-500 dark:text-zinc-400">Oct 2012 - Oct 2016</span>
                          </div>
                          <div className="w-[38.6%] text-left pl-2">
                            <span className="block text-rose-500 font-bold">Caminheiros</span>
                            <span className="text-[9px] font-mono text-zinc-500 dark:text-zinc-400">Oct 2016 - Oct 2021</span>
                          </div>
                        </div>
                      </div>
                    )}

                    {/* RODAPÉ ALINHADO E REFINADO PARA OS ESCOTEIROS */}
                    {selectedJob.company === "Corpo Nacional de Escutas" && (
                      <div className="mt-4 pt-3 border-t border-zinc-200 dark:border-white/10 flex items-center justify-between text-[11px] text-zinc-500 dark:text-zinc-400">
                        <span className="font-medium text-zinc-300">
                          Agrupamento 1334 - Santo António de Oliveirinha
                        </span>
                        <div className="h-7 w-7 rounded-full bg-white p-0.5 shadow-sm border border-zinc-700 flex items-center justify-center shrink-0 overflow-hidden">
                          <img src="/scout2.png" alt="Agrupamento 1334" className="h-full w-full object-contain rounded-full" />
                        </div>
                      </div>
                    )}
                    ...
                    {selectedJob.publishersTimeline && (
                      <div className="mt-4 pt-4 border-t border-zinc-200/50 dark:border-white/5">
                        <h4 className="text-[10px] font-bold uppercase tracking-[0.15em] text-zinc-800 dark:text-zinc-300 mb-3 flex items-center gap-2">
                          <Calendar className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> Timeline by Publisher
                        </h4>

...
// =========================================================
// 1. HELPER: CATEGORIAS ESPECÍFICAS (Tags Coloridas na Consola)
// =========================================================
...
function getSkillSubCategory(skillName: string) {
  ...
  // Circuit Design & Physics
  if (["kicad", "altium", "ltspice", "quartus", "pcb", "optoelectronic", "fpga", "lSC", "solar"].some(t => lower.includes(t))) 
    return subCategories["Circuit Design"];
  
  // Embedded & FPGA
  if (["arduino", "android", "verilog", "c/c++", "embedded", "intel"].some(t => lower.includes(t))) 
    return subCategories["Embedded Systems"];
  
  // Data & Viz
  if (["python", "matlab", "spark", "origin", "data"].some(t => lower.includes(t))) 
    return subCategories["Data & Viz"];
  
  // Dev Tools & General
  if (["visual studio", "git", "office", "latex", "java", "kotlin"].some(t => lower.includes(t))) 
...
// =========================================================
// 2. LOGICA DE PROCURA (O Cérebro)
// =========================================================
function findSkillUsage(skill: string) {
  const usage: any[] = [];
  // Limpar flags e parenteses para a busca funcionar (ex: "🇵🇹 Portuguese (Native)" -> "Portuguese")
  ...
  return Array.from(new Set(usage.map(u => JSON.stringify(u))))
    .map(s => JSON.parse(s))
    .slice(0, 4);
}

// =========================================================
// 3. CATEGORIAS DE SKILLS (Reorganizadas)
// =========================================================
const skillCategories = [
  ...
  },
  {
    ...
    icon: Cpu, // Usamos CPU para representar o "Heavy Machinery" de software (Altium, Quartus, etc)
    color: "blue",
    items: skills.toolsAndSoftware
  },
  {
    id: "os",
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="relative group">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {cat.items.map((skill) => {
                      ...
                      // Remove emojis e parenteses longos apenas visualmente se for preciso,
                      // mas aqui deixamos o nome completo para as Linguas ficarem bonitas
          ...
          <div className="lg:col-span-5 relative">
            <div className="sticky top-24">
              <GlassCard className="min-h-[550px] border border-zinc-200 dark:border-white/10 bg-zinc-50/80 dark:bg-[#0c0c0e]/80 backdrop-blur-xl overflow-hidden flex flex-col shadow-2xl">
                  ...
                  <AnimatePresence mode="wait">
                    {activeSkill ? (
                      ...
                      >
                        ...
                        <div className="mb-4 pb-3 border-b border-zinc-200 dark:border-white/5 flex flex-col gap-2">
                          ...
                          <div className="flex flex-col gap-2">
                            {/* Título com suporte a quebra de linha */}
                            ...
                            {subCategory && (
                              <div className={`
                                self-start flex items-center gap-1.5 px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider border
                                ${subCategory.color} ${subCategory.bg} ${subCategory.border}
                              `}>
                            ...
                            usageData.map((item: any, i: number) => (
                              <div key={i} className="group flex items-start gap-3 p-3 rounded-lg bg-white/50 dark:bg-white/5 border border-zinc-200/50 dark:border-white/5 hover:border-emerald-500/30 transition-all">
                                <div className={`mt-1 p-1.5 rounded-md shrink-0 
                                  ${item.type === 'experience' ? 'bg-blue-500/10 text-blue-500' : 
                                    item.type === 'education' ? 'bg-emerald-500/10 text-emerald-500' : 
                                ...
                                <div className="min-w-0">
                                  ...
                                  <p className="text-[10px] text-zinc-500 dark:text-zinc-400 leading-tight line-clamp-2">
                                    {item.subtitle}
                                  </p>
                                </div>
                              </div>
                      ...
                      >
                        ...
                        <p className="text-xs text-zinc-400 mt-1 max-w-[200px]">Hover over the technical skills to view execution context.</p>
                      </motion.div>
                    )}
                  </AnimatePresence>
                </div>
                ...
                <div className="p-2 border-t border-zinc-200 dark:border-white/10 bg-zinc-50 dark:bg-black/20 text-[9px] font-mono text-zinc-400 flex justify-between items-center">
                  {activeSkill ? (
                     <span className="text-emerald-600 dark:text-emerald-400 font-bold">
                       {usageData.length > 0 ? `${usageData.length} REFERENCES FOUND` : "CORE COMPETENCE"}
                     </span>
                  ) : (
